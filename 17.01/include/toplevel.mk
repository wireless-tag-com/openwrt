...
prepare-tmpinfo: FORCE
	...
	mkdir -p tmp/info
	$(_SINGLE)$(NO_TRACE_MAKE) -j1 -r -s -f include/scan.mk SCAN_TARGET="packageinfo" SCAN_DIR="package" SCAN_NAME="package" SCAN_DEPS="$(TOPDIR)/include/package*.mk $(TOPDIR)/overlay/*/*.mk" SCAN_DEPTH=10 SCAN_EXTRA=""
...
%_wt: scripts/config/conf prepare-tmpinfo FORCE
	if [ \! -e $(TOPDIR)/wtconfig/$(@:_wt=) ] ; then \
		$(_SINGLE)$(NO_TRACE_MAKE) menuconfig $(PREP_MK); \
	fi
	$(TOPDIR)/scripts/wtconfig.sh $(TOPDIR)/wtconfig $(TOPDIR)/wtconfig/$(@:_wt=) $(TOPDIR)/.config
	$< --defconfig=.config Config.in
